			...
			<div class="col-lg-12">
				<div class="headtop">
					<ul>
						...
						<li>
							<a href="mailto:{{ contacth()->email }}"><i class="fas fa-envelope"></i> {{ contacth()->email }}</a>
						</li>
						<li>
		...
		<div class="row">
			...
			<div class="col-lg-3 logo">
				<a href="{{ route('home.index') }}">
					<img src="{{ asset('public') }}/images/{{ contacth()->logo ?? 'notfind.png' }}" width="100%" height="auto" alt="logo">
				</a>
			</div>
			<!-- search -->
		...
		<div class="collapse navbar-collapse">
			<ul class="navbar-nav navbar-light">
				...
				@if($menu->slug=='interior-design-services')
					<li class="nav-item"><a class="nav-link" href="{{ url('category') }}/{{ $menu->slug }}">{{ $menu->title }}</a></li>
				@else
				<li class="nav-item dropdown megamenu-li dmenu">

					<a class="nav-link" href="{{ url('category') }}/{{ $menu->slug }}" id="dropdown01" >{{ $menu->title }}</a>
					<div class="dropdown-menu megamenu sm-menu border-top" aria-labelledby="dropdown01">
						<div class="row">
							@php
							$subcats=\App\Models\Category::where('parent_id',$menu->id)->get();
                            $brands=\App\Models\Product::groupBy('brand_id')->select('brand_id')->where('category_id',$menu->id)->get();
                            $colorss=\App\Models\Product::where('category_id',$menu->id)->where('color','!=',NULL)->get();
                            $sizess=\App\Models\Product::where('category_id',$menu->id)->where('size','!=',NULL)->get();
                            $bladess=\App\Models\Product::select('blade')->where('category_id',$menu->id)->where('blade','!=',NULL)->get();
							@endphp
							@if(count($subcats)>0)
							<div class="col-sm-6 col-lg-2 mb-4">
								<h6>TYPE</h6>
								@foreach($subcats as $subcat)
									<a class="dropdown-item" href="{{ url('category') }}/{{ $subcat->slug }}">{{ $subcat->title }}</a>
								@endforeach
							</div>
							@endif
							@if(count($brands)>0)
							<div class="col-sm-6 col-lg-2 mb-4">
								...
								@foreach($brands as $brand)
									<a class="dropdown-item" href="{{ url('shops') }}/{{ $menu->slug }}/{{ $brand->brand->slug ?? '#' }}">{{ $brand->brand->title ?? 'N/A' }}</a>
								@endforeach
							</div>
							@endif
							@if(count($colorss)>0)
							<div class="col-sm-6 col-lg-2 mb-4">
								...
								@foreach($color as $colorsr)
									...
									@endphp
									@if($colorr!=NULL)
										<a class="dropdown-item" href="{{ url('collection') }}/{{ $menu->slug }}/{{ $colorr->slug }}">
											<i class="fas fa-solid fa-circle" style="color: {{ $colorr->value }}"></i>{{ $colorr->name }}
										</a>
									@endif
								@endforeach
							</div>
							...
							<div class="col-sm-6 col-lg-2 mb-4">
								<h6>PRICE</h6>
								<a class="dropdown-item" href="{{ url('price') }}/{{ $menu->slug }}/0-5000">TK. 5000</a>
								<a class="dropdown-item" href="{{ url('price') }}/{{ $menu->slug }}/5001-10000">TK 5001 - TK10000</a>
								<a class="dropdown-item" href="{{ url('price') }}/{{ $menu->slug }}/10001-20000">TK 10001 - TK 20000</a>
								<a class="dropdown-item" href="{{ url('price') }}/{{ $menu->slug }}/20001-50000">TK 20001- TK 50000</a>
								<a class="dropdown-item" href="{{ url('price') }}/{{ $menu->slug }}/50001-100000">TK 50001 - TK 100000</a>
								<a class="dropdown-item" href="{{ url('price') }}/{{ $menu->slug }}/100001-100000">Up TK 100001</a>
							</div>
							@if(count($sizess)>0)
							<div class="col-sm-6 col-lg-2 mb-4">
								<h6>SIZE</h6>
								...
								@foreach($size as $sizesr)
									...
									@endphp
									@if($sizer!=NULL)
										<a class="dropdown-item" href="{{ url('collection') }}/{{ $menu->slug }}/{{ $sizer->slug }}">
											{{ $sizer->name }}
										</a>
									@endif
								@endforeach
...
<header id="header-area" class="navbar-demo">
    ...
    <div class="cartcom d-flex justify-content-end">
        <a href="{{ route('cart.list') }}"> <i class="fas fa-shopping-cart"></i> <span>{{ $cartCount }}</span></a>
    </div>
	<div class="search">
		{!! Form::open(['method'=>'POST','route'=>'search']) !!}
		<div class="input-group ">
