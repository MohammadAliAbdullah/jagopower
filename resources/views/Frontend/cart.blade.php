					...
					<table class="table table-light table-borderless table-hover text-center mb-0">
						...
						<tr class="table-bordered">
							...
							<td class="align-middle algimg">
								<img src="{{ asset('public/images/product/'.$item->attributes['image']) }}" alt="" style="width: 50px;">
							</td>
							<td class="align-middle">Tk {{$item->price ?? 0 }}</td>
							<td class="align-middle">
								{!! Form::open(['method'=>'POST','route'=>'update.cart']) !!}
								<div class="input-group cartt">
									<input type="hidden" value="{{ $item->id }}" class="form-control id" name="id"/>
							...
							<td class="align-middle">
								<a href="{{ route('cart.remove', $item->id) }}" class="btn btn-sm btn-danger"><i class="fa fa-times" aria-hidden="true"></i></a>
